...
class GameState:
    ...
    @property
    def xp_needed(self):
        if self.level < len(LEVEL_XP):
            return LEVEL_XP[self.level]
        return 9999
    ...
    def buy(self, source_id):
        """Intenta comprar una unidad de la fuente indicada."""
    ...
    def next_turn(self):
        ...
        if self.pollution > POLLUTION_LIMIT:
            self.money -= POLLUTION_FINE
            events.append((f"Multa por contaminación: -€{POLLUTION_FINE}", "bad"))

        # Game over
        if self.money < 0:
            self.game_over = True
            events.append(("Game Over: te has quedado sin dinero.", "bad"))

        for msg, color_key in events:
            self._log(msg, color_key)
